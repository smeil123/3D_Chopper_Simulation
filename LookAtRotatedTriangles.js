...
var VSHADER_SOURCE =
  ...
  'void main() {\n' +
  '  gl_Position = u_ViewMatrix * u_ModelMatrix * a_Position;\n' + //modelmatrix 적용 후 -> viewmatrix적용 (비효율적이다)
  '  v_Color = a_Color;\n' +
  '}\n';
...
function main() {
  ...
  // ModelMatrix -> object의 scale, rotate, translate, project
  // ViewMatrix -> camera의 look-at, up direction (카메라의 view를 조정하는 것 같지만 실제로는 object의 위치를 반대로 이동하는 것이다)
...
function initVertexBuffers(gl) {
  // z값이 작을수록 멀리 그려져야 한다.
  // 각 vertex가 parallel하게 그려진뒤 삼각형이 그려지기 때문에
  // 어떤 삼각형이 먼저 그려질지는 모른다.
  // 여기서는 좌표순서가 달라지면 그림도 달라지게 된다 (뒷부분에서 배움)
